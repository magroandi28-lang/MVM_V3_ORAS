...
def elemzes(dam_target,edf,data,target_atlag,nap_cimke):
    ...
    # ================= MEGÚJULÓ ENERGIA — TERV SZERINTI V2: NAP OSZLOP + SZÉL VONAL + JOBB INFÓSÁV =================
    tfc = data.get("target_fc")
    if tfc and tfc.get("nap") and tfc.get("szel"):
        nap_mw = [float(v) for v in tfc["nap"]][:24]
        szel_mw = [float(v) for v in tfc["szel"]][:24]
        x = list(range(24))
        ...
        total_safe = max(ossz_mwh, 1.0)
        nap_resz = nap_mwh / total_safe * 100
        szel_resz = szel_mwh / total_safe * 100

        def _num(v):
            return f"{v:,.0f}".replace(",", " ")

        def _base_layout(fig, ymax, height, ytitle="MW"):
            fig.update_layout(
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)",
                font=dict(color=C['mut'], family="Inter, sans-serif", size=10),
                margin=dict(l=48, r=14, t=8, b=28),
                height=height,
                showlegend=False,
                hovermode="x unified",
            )
        ...
        # --- Napenergia: a terv szerint oszlopdiagram, nem terület/domb ---
        fig_nap = go.Figure()
        nap_ymax = max(nap_mw) * 1.32 if max(nap_mw) > 0 else 10
        bar_cols = []
        m = max(nap_mw) if max(nap_mw) > 0 else 1.0
        for v in nap_mw:
            opacity = 0.32 + 0.68 * (v / m)
            bar_cols.append(f"rgba(255,176,32,{opacity:.2f})")
        fig_nap.add_trace(go.Bar(
            x=x, y=nap_mw, width=0.42,
            marker=dict(color=bar_cols, line=dict(color="rgba(255,220,130,0.65)", width=0.6)),
            hovertemplate="%{x:02d}:00<br>%{y:,.0f} MW<extra>Napenergia</extra>",
        ))
        fig_nap.add_trace(go.Scatter(
            x=[nap_max_i], y=[nap_max], mode="markers",
            marker=dict(size=11, color="#fff3c4", line=dict(width=3, color="#ffb020")),
            hoverinfo="skip", showlegend=False,
        ))
        fig_nap.add_annotation(
            x=nap_max_i, y=nap_max,
            text=f"<b>{nap_max_i:02d}:00</b><br>{_num(nap_max)} MW",
            showarrow=True, arrowhead=0, ax=0, ay=-40,
            bgcolor="#0b1628", bordercolor="#ffb020", borderwidth=1, borderpad=6,
            font=dict(color=C['wh'], size=10),
        )
        fig_nap = _base_layout(fig_nap, nap_ymax, 170, "MW")

        # --- Szélenergia: külön vonalgrafikon, finom kitöltéssel ---
        fig_szel = go.Figure()
        szel_ymax = max(szel_mw) * 1.45 if max(szel_mw) > 0 else 10
        fig_szel.add_trace(go.Scatter(
            x=x, y=szel_mw, mode="lines",
            line=dict(color="rgba(42,215,232,0.20)", width=12, shape="spline", smoothing=0.45),
            hoverinfo="skip", showlegend=False,
        ))
        fig_szel.add_trace(go.Scatter(
            x=x, y=szel_mw, mode="lines+markers",
            line=dict(color="#2ad7e8", width=2.4, shape="spline", smoothing=0.45),
            marker=dict(size=5, color="#b9fbff", line=dict(width=1.4, color="#2ad7e8")),
            fill="tozeroy", fillcolor="rgba(42,215,232,0.16)",
            hovertemplate="%{x:02d}:00<br>%{y:,.0f} MW<extra>Szélenergia</extra>",
        ))
        fig_szel.add_trace(go.Scatter(
            x=[szel_max_i], y=[szel_max], mode="markers",
            marker=dict(size=11, color="#e7feff", line=dict(width=3, color="#2ad7e8")),
            hoverinfo="skip", showlegend=False,
        ))
        fig_szel.add_annotation(
            x=szel_max_i, y=szel_max,
            text=f"<b>{szel_max_i:02d}:00</b><br>{_num(szel_max)} MW",
            showarrow=True, arrowhead=0, ax=0, ay=-38,
            bgcolor="#0b1628", bordercolor="#2ad7e8", borderwidth=1, borderpad=6,
            font=dict(color=C['wh'], size=10),
        )
        # kis, visszafogott turbinajelölők csak néhány ponton, dekorációként
        for i in sorted(set([szel_max_i, 4, 8, 20])):
            fig_szel.add_annotation(
                x=i, y=szel_mw[i], text="✣", showarrow=False,
                yshift=24, font=dict(size=20, color="rgba(210,250,255,0.86)"),
                opacity=0.9,
            )
        fig_szel = _base_layout(fig_szel, szel_ymax, 170, "MW")

        def info_row(ikon, cim, ertek, alcim, szin):
            return html.Div([
                html.Div(ikon, style={"fontSize":"24px", "lineHeight":"1", "width":"34px", "color":szin, "textAlign":"center"}),
                html.Div([
                    html.Div(cim, style={"fontSize":"10px", "fontWeight":"800", "textTransform":"uppercase", "letterSpacing":"0.04em", "color":"#94a3b8"}),
                    html.Div(ertek, style={"fontSize":"18px", "fontWeight":"850", "color":szin, "marginTop":"2px"}),
                    html.Div(alcim, style={"fontSize":"10px", "color":C['mut'], "marginTop":"1px"}),
                ], style={"flex":"1"})
            ], style={
        ...
        info_panel = html.Div([
            html.Div("MAI ÖSSZESÍTÉS", style={"fontSize":"13px", "fontWeight":"850", "color":C['wh'], "marginBottom":"8px"}),
            info_row("☀", "Napenergia összesen", f"{_num(nap_mwh)} MWh", "a kiválasztott napra", "#ffb020"),
            info_row("⌁", "Szélenergia összesen", f"{_num(szel_mwh)} MWh", "a kiválasztott napra", "#2ad7e8"),
            info_row("◇", "Összes megújuló", f"{_num(ossz_mwh)} MWh", "nap + szél", "#a855f7"),
            info_row("☀", "Nap csúcs", f"{_num(nap_max)} MW", f"{nap_max_i:02d}:00", "#ffb020"),
            info_row("⌁", "Szél csúcs", f"{_num(szel_max)} MW", f"{szel_max_i:02d}:00", "#2ad7e8"),
            html.Div([
                html.Div("NAP / SZÉL ARÁNY", style={"fontSize":"10px", "fontWeight":"800", "letterSpacing":"0.04em", "color":"#94a3b8", "marginBottom":"7px"}),
                html.Div(f"Nap {nap_resz:.0f}%  ·  Szél {szel_resz:.0f}%", style={"fontSize":"18px", "fontWeight":"850", "color":C['wh'], "marginBottom":"8px"}),
                html.Div([
                    html.Div(style={"width":f"{nap_resz:.1f}%", "height":"8px", "background":"#ffb020", "borderRadius":"999px 0 0 999px"}),
                    html.Div(style={"width":f"{szel_resz:.1f}%", "height":"8px", "background":"#2ad7e8", "borderRadius":"0 999px 999px 0"}),
                ], style={"display":"flex", "height":"8px", "background":"rgba(148,163,184,0.16)", "borderRadius":"999px", "overflow":"hidden"}),
            ], style={"padding":"12px 0 0"}),
            html.Div(f"Forrás: ENTSO-E nap/szél előrejelzés · Frissítve: {data.get('frissites','–')}",
                style={"fontSize":"10px", "color":C['mut'], "marginTop":"14px"})
        ], style={
            "background":"rgba(7,18,35,0.80)",
        ...
        termeles_resz = [
            html.Div([
                html.Div("MEGÚJULÓ ENERGIA", style={"fontSize":"14px", "fontWeight":"850", "color":C['wh']}),
            ...
            html.Div([
                html.Div([
                    html.Div([
                        html.Div([
                            html.Span("☀", style={"color":"#ffb020", "fontSize":"17px", "marginRight":"8px"}),
                            html.Span("NAPENERGIA", style={"fontSize":"12px", "fontWeight":"850", "color":C['wh']})
                        ], style={"display":"flex", "alignItems":"center", "marginBottom":"2px"}),
                        dcc.Graph(figure=fig_nap, config={"displayModeBar":False}, style={"height":"170px"}),
                    ], style={"background":"rgba(8,19,36,0.42)", "border":"1px solid rgba(148,163,184,0.09)", "borderRadius":"12px", "padding":"10px 10px 2px"}),
                    html.Div([
                        html.Div([
                            html.Span("⌁", style={"color":"#2ad7e8", "fontSize":"17px", "marginRight":"8px"}),
                            html.Span("SZÉLENERGIA", style={"fontSize":"12px", "fontWeight":"850", "color":C['wh']})
                        ], style={"display":"flex", "alignItems":"center", "marginBottom":"2px"}),
                        dcc.Graph(figure=fig_szel, config={"displayModeBar":False}, style={"height":"170px"}),
                    ], style={"background":"rgba(8,19,36,0.42)", "border":"1px solid rgba(148,163,184,0.09)", "borderRadius":"12px", "padding":"10px 10px 2px"}),
                ], style={"display":"grid", "gridTemplateRows":"1fr 1fr", "gap":"10px", "minWidth":"0"}),
                html.Div(info_panel, style={"minWidth":"275px"})
            ], style={
                "display":"grid",
                "gridTemplateColumns":"minmax(0, 1fr) 295px",
                "gap":"14px",
                "alignItems":"stretch"
            })
        ]
