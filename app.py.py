...
def _mini_sparkline(trend, szin):
    """Alsó mini vonaldiagram: külön zónában, nem húzza át a szöveget."""
    if trend is None:
        return None
    ...
    fig.add_trace(go.Scatter(
        ...
        fill="tozeroy",
        fillcolor=_rgba(szin, 0.07),
        hoverinfo="skip",
        showlegend=False
    ))
    fig.add_trace(go.Scatter(
        x=[len(y)-1], y=[y[-1]], mode="markers",
        marker=dict(size=7, color=szin, line=dict(width=1.1, color="rgba(255,255,255,.75)")),
        hoverinfo="skip",
        showlegend=False
    ))
    fig.update_layout(
...
def _budapest_silhouette():
    """Külső fájl nélküli Országház + híd sziluett a Budapest-kártyához."""
    ...
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 620 170">
      <defs>
        <linearGradient id="g" x1="0" x2="0" y1="0" y2="1">
          ...
          <stop offset="70%" stop-color="#f97316" stop-opacity="0.34"/>
      ...
      <g opacity="0.72" filter="url(#glow)">
        <path d="M12 122 C90 78, 142 78, 220 122" fill="none" stroke="#f59e0b" stroke-width="3" opacity="0.58"/>
        <path d="M25 122 L25 95 M206 122 L206 95" stroke="#f59e0b" stroke-width="4" opacity="0.48"/>
        <path d="M12 124 H232" stroke="#f59e0b" stroke-width="2" opacity="0.42"/>
        <path d="M50 123 L82 96 L114 123 M124 123 L158 96 L194 123" stroke="#f59e0b" stroke-width="1.8" opacity="0.36"/>
        <g transform="translate(220,18)">
          <path d="M0 105 H310 V118 H0 Z" fill="url(#g)" opacity="0.82"/>
          <path d="M18 70 H292 V106 H18 Z" fill="url(#g)" opacity="0.68"/>
          <path d="M130 106 V48 C130 26,180 26,180 48 V106 Z" fill="url(#g)" opacity="0.92"/>
          <path d="M138 48 C148 10,162 10,172 48 Z" fill="#f59e0b" opacity="0.68"/>
          <path d="M154 6 V48" stroke="#f59e0b" stroke-width="3" opacity="0.74"/>
          <path d="M35 106 V55 H62 V106 Z M80 106 V42 H105 V106 Z M205 106 V42 H230 V106 Z M248 106 V55 H275 V106 Z" fill="url(#g)" opacity="0.74"/>
          <path d="M48 55 L48 30 M92 42 L92 18 M217 42 L217 18 M262 55 L262 30" stroke="#f59e0b" stroke-width="3" opacity="0.64"/>
          <path d="M36 55 L48 28 L60 55 M80 42 L92 16 L104 42 M205 42 L217 16 L229 42 M250 55 L262 28 L274 55" fill="#f59e0b" opacity="0.44"/>
        </g>
        <path d="M0 138 C90 128, 160 148, 250 136 C365 122, 500 152, 620 135" fill="none" stroke="#f59e0b" stroke-width="2" opacity="0.16"/>
        <path d="M80 154 C170 144, 230 160, 330 150 C430 140, 520 164, 600 150" fill="none" stroke="#f97316" stroke-width="1.6" opacity="0.12"/>
      </g>
    </svg>
    """
...
def _fx_texture(szin):
    return html.Div(style={
        ...
        "zIndex": "1",
        "pointerEvents": "none",
        "backgroundImage": f"radial-gradient(circle, {_rgba(szin, 0.72)} 1px, transparent 1.6px)",
...
def kpi(cim, val, sub, szin, trend=None):
    """Prémium fekvő KPI-kártya. A meglévő kpi(...) hívások változatlanul működnek."""
    ...
    if "dam" in cim_lower or "fogyasztás" in cim_lower:
        vizual = _mini_sparkline(trend, szin)
    ...
    return html.Div([
        html.Div(style={
            ...
            "background":szin, "boxShadow":f"0 0 9px {_rgba(szin,.85)}", "zIndex":"4"
        }),
        html.Div([
            html.Div(cim, className="kpi-label", style={
                "position":"relative", "zIndex":"3", "marginBottom":"7px",
                "whiteSpace":"nowrap", "overflow":"hidden", "textOverflow":"ellipsis"
            }),
            html.Div([
                html.Span(value, className="kpi-value", style={
                    "display":"inline", "position":"relative", "zIndex":"3", "lineHeight":"1"
                }),
                html.Span(f" {unit}" if unit else "", style={
                    "fontSize":"15px", "fontWeight":"700", "color":C['wh'],
                    "position":"relative", "zIndex":"3", "lineHeight":"1"
                })
            ], style={
                "position":"relative", "zIndex":"3", "whiteSpace":"nowrap",
                "overflow":"hidden", "textOverflow":"ellipsis", "marginBottom":"6px"
            }),
            html.Div(sub, className="kpi-sub", style={
                "position":"relative", "zIndex":"3", "color":szin,
                "whiteSpace":"nowrap", "overflow":"hidden", "textOverflow":"ellipsis"
            })
        ], style={"position":"relative", "zIndex":"3", "paddingRight":"16px"}),
        ...
        "background": f"radial-gradient(circle at 92% 14%, {_rgba(szin,.14)} 0%, rgba(5,13,26,0) 28%), linear-gradient(180deg, rgba(10,22,40,.98) 0%, rgba(7,15,30,.96) 100%)",
