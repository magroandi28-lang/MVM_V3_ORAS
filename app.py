...
# ============================================================
# KPI KÁRTYÁK — prémium fekvő sor, átfedés nélkül
# ============================================================
KPI_GRID_STYLE = {
    "display": "grid",
...
def _rgba(hex_color, alpha):
    """#RRGGBB → rgba(). Ha hibás színt kap, biztonságosan fehérre esik vissza."""
    try:
...
def _mini_sparkline(trend, szin):
    ...
    fig.add_trace(go.Scatter(
        y=y,
        mode="lines",
        line=dict(color=szin, width=2.35, shape="spline"),
        fill="tozeroy",
...
def _quality_bars(szin, val):
    try:
        ...
        aktiv_db = 0
    aktiv = min(22, max(1, math.ceil(aktiv_db / 3))) if aktiv_db > 0 else 0
...
def kpi(cim, val, sub, szin, trend=None):
    """Prémium fekvő KPI-kártya. A régi kpi(...) hívások változatlanul működnek."""
    ...
    value_len = len(value) + len(unit)
    value_size = "24px" if value_len > 13 else ("28px" if value_len > 9 else "33px")
    unit_size = "14px" if value_len > 13 else "16px"
    ...
    card_style = {
        ...
        "background": (
            f"radial-gradient(circle at 91% 12%, {_rgba(szin,.18)} 0%, rgba(5,13,26,0) 25%),"
            "linear-gradient(180deg, rgba(9,22,39,.98) 0%, rgba(4,14,28,.97) 100%)"
        ),
        "border": "1px solid rgba(39,102,157,.42)",
        "boxShadow": (
            f"0 0 0 1px {_rgba(szin,.045)}, "
            f"0 0 18px rgba(14,72,128,.18), "
            f"inset 0 0 24px {_rgba(szin,.055)}"
        ),
    }
    ...
    return html.Div([
        html.Div(style={
            ...
            "borderRadius": "999px",
            "background": szin,
            "boxShadow": f"0 0 9px {_rgba(szin,.85)}",
            "zIndex": "4"
        }),
        html.Div([
