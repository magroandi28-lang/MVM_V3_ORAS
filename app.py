...
def _budapest_silhouette():
    return html.Div(style={
        ...
        "backgroundImage": f"url({app.get_asset_url('bp-parliament.png')})",
...
def _fx_texture(szin):
    return html.Div(style={
        "position":"absolute","left":"0","right":"0","bottom":"0","height":"80px",
        "zIndex":"1","pointerEvents":"none",
        "backgroundImage": f"url({app.get_asset_url('eur-wave.png')})",
...
def kpi(cim, val, sub, szin, trend=None, jelolt_i=None):
    ...
    elif "budapest" in cim_lower:
        vizual = _budapest_silhouette()
    elif "eur" in cim_lower:
        vizual = _fx_texture(szin)
    ...
    return html.Div([
        ...
        vizual if vizual else html.Div()
...
def render(data,oldal,_clock):
    ...
    if aj:
        ...
        legolcs_txt = f"{_nap_elotag(k)}{k:%H:%M} · {aj['aj_ar']:.0f} €"
    ...
    ksor=html.Div([
        ...
        kpi("Legolcsóbb ablak",legolcs_txt,"Publikált árakból",C['gr']),
...
def elemzes(edf, data):
    ...
    y_felso = y_hi + y_ter*0.22           # hely a zóna-feliratoknak
    ...
    fig = make_subplots(rows=2, cols=1, shared_xaxes=True,
                        vertical_spacing=0.07, row_heights=[0.62, 0.38])
    ...
    while i < N:
        ...
        if j - i >= 1:
            fig.add_annotation(x=(i+j)/2, y=y_felso - y_ter*0.03, text=nev,
                showarrow=False, font=dict(size=10, color=fg), row=1, col=1)
        i = j + 1
    ...
    fig.add_annotation(x=i_max,y=y[i_max],
        ...
        showarrow=True,arrowhead=0,ax=-64,ay=-34,bgcolor="#0f1d31",
    ...
    fig.update_layout(paper_bgcolor='rgba(0,0,0,0)',plot_bgcolor='rgba(0,0,0,0)',
        font=dict(color=C['mut'],family='Inter,sans-serif',size=10),
        margin=dict(l=52,r=18,t=16,b=30),height=520,showlegend=False,hovermode="closest")
    ...
    fogy_panel = html.Div([
        ...
        dcc.Graph(figure=fig,config={"displayModeBar":False},style={"height":"520px"})
    ], style=CS)
...
def _megujulo_panel(meg, dtok, cim):
    ...
    # az utolsó óra, amelyre van mérés
    mert_ig = max([i for i,v in enumerate(t_nap) if v is not None], default=-1)

    fig = make_subplots(specs=[[{"secondary_y": True}]])

    # --- NAP: jóslat (halvány, szaggatott, kitöltve) ---
    fig.add_trace(go.Scatter(x=mx, y=f_nap, mode="lines", name="Nap · jóslat",
        line=dict(color=NAP, width=1.8, dash="dot"), opacity=0.75,
        fill="tozeroy", fillcolor="rgba(245,158,11,0.07)",
        customdata=[cimke(t) for t in mdtok],
        hovertemplate="%{customdata}<br>%{y:,.0f} MW<extra>Nap · jóslat</extra>"),
        secondary_y=False)

    # --- NAP: mért (telt, izzó) ---
    if mert_ig >= 0:
        xs = mx[:mert_ig+1]; ys = t_nap[:mert_ig+1]
        for r_,o_ in [(13,0.07),(8,0.13)]:
            fig.add_trace(go.Scatter(x=xs, y=ys, mode="markers",
                marker=dict(size=r_, color=NAP_TENY, opacity=o_),
                hoverinfo="skip", showlegend=False), secondary_y=False)
        fig.add_trace(go.Scatter(x=xs, y=ys, mode="lines", name="Nap · mért",
            line=dict(color=NAP_TENY, width=2.6),
            customdata=[cimke(t) for t in mdtok[:mert_ig+1]],
            hovertemplate="%{customdata}<br>%{y:,.0f} MW<extra>Nap · mért</extra>"),
            secondary_y=False)

    # --- SZÉL: jóslat és mért, másodlagos tengelyen ---
    fig.add_trace(go.Scatter(x=mx, y=f_szel, mode="lines", name="Szél · jóslat",
        line=dict(color=SZEL, width=1.6, dash="dot"), opacity=0.7,
        customdata=[cimke(t) for t in mdtok],
        hovertemplate="%{customdata}<br>%{y:,.0f} MW<extra>Szél · jóslat</extra>"),
        secondary_y=True)
    if mert_ig >= 0:
        xs = mx[:mert_ig+1]; ys = t_szel[:mert_ig+1]
        for r_,o_ in [(11,0.07),(7,0.13)]:
            fig.add_trace(go.Scatter(x=xs, y=ys, mode="markers",
                marker=dict(size=r_, color=SZEL_TENY, opacity=o_),
                hoverinfo="skip", showlegend=False), secondary_y=True)
        fig.add_trace(go.Scatter(x=xs, y=ys, mode="lines", name="Szél · mért",
            line=dict(color=SZEL_TENY, width=2.2),
            customdata=[cimke(t) for t in mdtok[:mert_ig+1]],
            hovertemplate="%{customdata}<br>%{y:,.0f} MW<extra>Szél · mért</extra>"),
            secondary_y=True)

    nap_top = max(max(f_nap), max([v for v in t_nap if v is not None], default=0)) * 1.20 or 1.0
    szel_top = max(max(f_szel), max([v for v in t_szel if v is not None], default=0)) * 1.45 or 1.0

    # --- turbina-motívum a szél görbén, három ponton ---
    pole_h = szel_top * 0.070; blade_h = pole_h * 0.75; bdx = M * 0.012
    for k in [i for i in (M//5, M//2, 4*M//5) if 0 <= i < M]:
        alap = f_szel[k] + pole_h * 0.4; hub = alap + pole_h
        for x0,x1,y0,y1 in [(k,k,alap,hub), (k,k,hub,hub+blade_h),
                            (k,k+bdx,hub,hub-blade_h*0.5), (k,k-bdx,hub,hub-blade_h*0.5)]:
            fig.add_shape(type="line", x0=x0, x1=x1, y0=y0, y1=y1, yref="y2",
                line=dict(color="#a9ecf5", width=1))

    # --- "most" elválasztó ---
    if 0 <= mert_ig < M-1:
        fig.add_shape(type="line", x0=mert_ig+0.5, x1=mert_ig+0.5, y0=0, y1=1,
            yref="paper", line=dict(color=C['wh'], width=1, dash="dot"))
        fig.add_annotation(x=mert_ig+0.5, y=1.0, yref="paper", yanchor="bottom",
            text="most", showarrow=False, font=dict(size=9, color="#94a3b8"))

    # --- éjfél ---
    ej = next((i for i in range(1,M) if mdtok[i].hour == 0), None)
    if ej:
        fig.add_shape(type="line", x0=ej-0.5, x1=ej-0.5, y0=0, y1=1, yref="paper",
            line=dict(color="#3c5873", width=1, dash="dash"))

    fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
        font=dict(color=C['mut'], family='Inter,sans-serif', size=10),
        margin=dict(l=48, r=48, t=34, b=30), height=280, hovermode="closest",
        legend=dict(orientation="h", yanchor="bottom", y=1.04, x=0,
            bgcolor="rgba(0,0,0,0)", font=dict(size=9, color=C['txt'])))
    ...
    fig.update_yaxes(title_text="Nap (MW)", range=[0, nap_top], gridcolor="#101f35",
        color=NAP, zeroline=False, fixedrange=True, secondary_y=False)
    fig.update_yaxes(title_text="Szél (MW)", range=[0, szel_top], showgrid=False,
        color=SZEL, zeroline=False, fixedrange=True, secondary_y=True)
    ...
    def _sav(nev, ossz, kulcs, szin, ikon):
        lo, hi = FC_SAV[kulcs]
        return html.Div([
            html.Span([html.Span(ikon, style={"color":szin,"marginRight":"5px"}), nev],
                style={"fontSize":"10px","color":C['mut']}),
            html.Span(f"{ossz*(1+lo):,.0f} – {ossz*(1+hi):,.0f}".replace(","," "),
                style={"fontSize":"10px","color":szin,"fontWeight":"500"})
        ], style={"display":"flex","justifyContent":"space-between","padding":"4px 2px"})

    nap_mwh = float(np.sum(f_nap)); szel_mwh = float(np.sum(f_szel))
    nap_csucs_i = int(np.argmax(f_nap)); szel_csucs_i = int(np.argmax(f_szel))
    ...
    osszesito = html.Div([
        ...
        html.Div("Előrejelzett csúcs", style={"fontSize":"9px","color":C['mut'],
            "marginBottom":"3px"}),
        html.Div([
            html.Span([html.Span("☀", style={"color":NAP,"marginRight":"5px"}), "Nap"],
                style={"fontSize":"10px","color":C['mut']}),
            html.Span(f"{cimke(mdtok[nap_csucs_i])} · {f_nap[nap_csucs_i]:,.0f} MW".replace(","," "),
                style={"fontSize":"10px","color":NAP,"fontWeight":"500"})
        ], style={"display":"flex","justifyContent":"space-between","padding":"4px 2px"}),
        html.Div([
            html.Span([html.Span("◇", style={"color":SZEL,"marginRight":"5px"}), "Szél"],
                style={"fontSize":"10px","color":C['mut']}),
            html.Span(f"{cimke(mdtok[szel_csucs_i])} · {f_szel[szel_csucs_i]:,.0f} MW".replace(","," "),
                style={"fontSize":"10px","color":SZEL,"fontWeight":"500"})
        ], style={"display":"flex","justifyContent":"space-between","padding":"4px 2px"}),
        html.Div(style={"borderTop":f"1px solid {C['brd']}","marginTop":"6px","marginBottom":"6px"}),
        html.Div("Termelés a sávban — tapasztalati tartomány (MWh)",
            style={"fontSize":"9px","color":C['mut'],"marginBottom":"3px"}),
        _sav("Nap", nap_mwh, "nap", NAP, "☀"),
        _sav("Szél", szel_mwh, "szel", SZEL, "◇"),
        html.Div("A napok 80%-a ebbe a sávba esett (8646 óra, 2025–2026).",
    ...
    return html.Div([
        ...
        html.Div(f"ENTSO-E napelőtti jóslat vs. mért termelés · a mai éjféltől {cim.lower()} végéig{mert_txt}",
